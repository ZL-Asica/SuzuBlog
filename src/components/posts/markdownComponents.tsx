...
const generateHierarchicalSlug = (
  ...
  // Create a hierarchical slug based on the current heading levels
  const hierarchicalSlug = Object.values(headingLevels)
    .slice(0, Object.keys(headingLevels).indexOf(level) + 1)
    .join('-');

  // Based on the current heading level, generate a base slug
  const baseSlug = slugify(String(children), { lower: true });
  return `${hierarchicalSlug}-${baseSlug}`;
};
...
const markdownComponents: Components = {
  ...
  ul: ({ children }) => (
    <ul className='my-4 list-inside list-disc pl-4'>{children as ReactNode}</ul>
  ),
  ol: ({ children }) => (
